...
async fn test_websocket_join_vote_and_reveal_flow() {
    ...
    ws_a.send(Message::Text(serde_json::to_string(&join_cmd_a).unwrap().into()))
        .await
        .unwrap();

    // 2. Client B (Estimator) connects
    let (mut ws_b, _) = connect_async(&ws_url).await.expect("Failed to connect ws_b");
    ...
    ws_b.send(Message::Text(serde_json::to_string(&join_cmd_b).unwrap().into()))
        .await
        .unwrap();

    // 3. Client A starts voting
    let start_voting_cmd = ClientCommand::StartVoting;
    ws_a.send(Message::Text(serde_json::to_string(&start_voting_cmd).unwrap().into()))
        .await
        .unwrap();

    // 4. Client A votes 5, Client B votes 8
    let vote_a = ClientCommand::CastVote { value: "5".to_string() };
    ws_a.send(Message::Text(serde_json::to_string(&vote_a).unwrap().into()))
        .await
        .unwrap();

    let vote_b = ClientCommand::CastVote { value: "8".to_string() };
    ws_b.send(Message::Text(serde_json::to_string(&vote_b).unwrap().into()))
        .await
        .unwrap();
    ...
    for _ in 0..10 {
        if let Some(Ok(Message::Text(text))) = ws_b.next().await {
            if let Ok(ServerEvent::RoomSnapshot { state }) = serde_json::from_str::<ServerEvent>(&text) {
    ...
    assert!(got_masked_snapshot, "Should have received snapshot verifying Reveal Gate masking");

    // 6. Client A reveals cards
    let reveal_cmd = ClientCommand::RevealCards;
    ws_a.send(Message::Text(serde_json::to_string(&reveal_cmd).unwrap().into()))
        .await
        .unwrap();
        ...
        if let Some(Ok(Message::Text(text))) = ws_b.next().await {
            if let Ok(ServerEvent::RoomSnapshot { state }) = serde_json::from_str::<ServerEvent>(&text) {
                if state.phase == server::domain::models::EstimationPhase::Revealed {
                    let p_a = state.participants.iter().find(|p| p.id == "user-a").unwrap();
                    let p_b = state.participants.iter().find(|p| p.id == "user-b").unwrap();
    ...
    assert!(got_revealed_snapshot, "Should have received unmasked snapshot upon reveal");
}
