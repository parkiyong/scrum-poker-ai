...
export const DeckSelector: React.FC<DeckSelectorProps> = ({
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-30 bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-2xl px-4 py-3 shadow-2xl flex items-center gap-2 max-w-full overflow-x-auto">
      <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider px-2 hidden md:inline">
        Pick Card:
      </span>
          ...
          return (
            <button
              ...
              className={`w-11 h-16 sm:w-12 sm:h-18 rounded-xl font-black text-base sm:text-lg flex flex-col justify-between p-1.5 transition-all duration-200 select-none shadow-md ${
                isSelected
                  ? 'bg-gradient-to-b from-indigo-500 to-indigo-700 text-white -translate-y-2.5 shadow-indigo-500/40 ring-2 ring-indigo-400'
                  : 'bg-slate-800 text-slate-200 border border-slate-700 hover:bg-slate-750 hover:border-slate-600 hover:-translate-y-1'
